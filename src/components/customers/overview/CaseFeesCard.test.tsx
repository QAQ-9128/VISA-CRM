...
describe('CaseFeesCard — 本案支出（重排：⋯ 菜单，无展开/无状态/无主操作）', () => {
  it('支出区：三类支出行（类型标签+备注+金额）+ 三类小计与合计；收款不混入；应收合计不受影响', async () => {
  ...
  it('支出行【没有】展开 chevron、【没有】状态 chip、【没有】记付款主操作（作用域到支出行）', async () => {
    renderCard([], [expCo]) // 一条支出（款项 IT 仍是待付款收款行，单独存在）
    await screen.findByText('本案支出')
    const expenseRow = screen.getByText('付主代理').closest('li') as HTMLElement
    const u = within(expenseRow)
    expect(u.queryByRole('button', { name: '展开收款明细' })).not.toBeInTheDocument()
    expect(u.queryByText('待付款')).not.toBeInTheDocument()
    expect(u.queryByText('已收款')).not.toBeInTheDocument()
    expect(u.queryByRole('button', { name: /记付款|记收款/ })).not.toBeInTheDocument()
    expect(u.getByRole('button', { name: '支出操作' })).toBeInTheDocument() // 只有 ⋯
  })

  it('支出行 ⋯ 含且仅含：修改支出 / 撤销·删除支出（不含「明细」）', async () => {
    renderCard([], [expCo])
    await screen.findByText('本案支出')
    openMenu('支出操作')
    const items = screen.getAllByRole('menuitem')
    expect(items.map((b) => b.textContent)).toEqual(['修改支出', '撤销·删除支出'])
    expect(screen.queryByRole('menuitem', { name: /明细/ })).not.toBeInTheDocument()
  })

  it('修改支出：⋯ → 修改支出 → 改金额 + 改付款对象(介绍人) → updatePayment 带新值（无日期字段）', async () => {
    renderCard([], [expCo]) // 付主代理 3000
    await screen.findByText('本案支出')
    openMenu('支出操作')
    fireEvent.click(screen.getByRole('menuitem', { name: '修改支出' }))
    const amt = screen.getByLabelText(/修改金额/) as HTMLInputElement
    expect(amt.value).toBe('3000')
    expect(screen.queryByLabelText(/修改日期/)).not.toBeInTheDocument() // 支出不录日期
    fireEvent.change(amt, { target: { value: '2500' } })
    fireEvent.change(screen.getByLabelText(/付款对象/), { target: { value: 'to_referrer' } })
    fireEvent.click(screen.getByRole('button', { name: '保存修改' }))
    await waitFor(() =>
      expect(payApi.updatePayment).toHaveBeenCalledWith('E1', { amount: 2500, direction: 'to_referrer' }),
    )
  })

  it('+ 记支出 单步：付款对象(介绍人)+金额+备注 → createPayment(to_referrer)；无实际日期字段、方式仅 转账/现金/垫付', async () => {
    renderCard([], [payment])
    fireEvent.click(await screen.findByRole('button', { name: '+ 记支出' }))
    // 单步：无「应付款项」步骤、无「款项类型(律师费/文案费/其他)」下拉
    expect(screen.queryByText('应付款项')).not.toBeInTheDocument()
    expect(screen.queryByText('款项类型')).not.toBeInTheDocument()
    expect(screen.queryByRole('option', { name: '律师费' })).not.toBeInTheDocument()
    // 支出不录实际日期
    expect(screen.queryByLabelText(/实际日期/)).not.toBeInTheDocument()
    // 方式只剩 转账 / 现金 / 垫付（无 微信/支付宝/卡/其他）
    const methods = within(screen.getByLabelText(/方式/)).getAllByRole('option').map((o) => o.textContent)
    expect(methods).toEqual(['转账', '现金', '垫付'])
    // 付款对象=介绍人 → to_referrer
    fireEvent.change(screen.getByLabelText(/付款对象/), { target: { value: 'to_referrer' } })
    fireEvent.change(screen.getByLabelText(/金额/), { target: { value: '800' } })
    fireEvent.change(screen.getByLabelText('备注（选填）'), { target: { value: '介绍佣金' } })
    fireEvent.click(screen.getByRole('button', { name: '保存支出' }))
    await waitFor(() =>
      expect(payApi.createPayment).toHaveBeenCalledWith(
        expect.objectContaining({
          case_id: 'ca1', direction: 'to_referrer', amount: 800, currency: 'AUD',
          note: '介绍佣金', applicant_id: null, plan_item_id: null,
        }),
      ),
    )
    const arg = vi.mocked(payApi.createPayment).mock.calls[0][0] as { paid_at?: string | null }
    expect(arg.paid_at).toMatch(/^\d{4}-\d{2}-\d{2}$/) // 自动取记账当天（本地澳洲日期）
  })

  it('付款对象=主代理 → to_company（双流：净额按 from_client − to_company − to_referrer 重算）', async () => {
    renderCard([], [payment])
    fireEvent.click(await screen.findByRole('button', { name: '+ 记支出' }))
    fireEvent.change(screen.getByLabelText(/付款对象/), { target: { value: 'to_company' } })
    fireEvent.change(screen.getByLabelText(/金额/), { target: { value: '3000' } })
    fireEvent.click(screen.getByRole('button', { name: '保存支出' }))
    await waitFor(() =>
      expect(payApi.createPayment).toHaveBeenCalledWith(
        expect.objectContaining({ case_id: 'ca1', direction: 'to_company', amount: 3000, plan_item_id: null, applicant_id: null }),
      ),
    )
  })
  ...
  it('本案净额 = 已收 − 支出合计（含垫付）：100 − 4150 = −4050', async () => {
    renderCard([], [payment, expCo, expRef, expMisc])
