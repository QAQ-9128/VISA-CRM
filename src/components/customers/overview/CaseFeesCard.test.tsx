...
describe('CaseFeesCard — 本案支出（重排：⋯ 菜单，无展开/无状态/无主操作）', () => {
  ...
  it('修改支出：⋯ → 修改支出 → 改金额 + 改收款方(付介绍人) → updatePayment 带新值', async () => {
    ...
    fireEvent.change(screen.getByLabelText('支出类型'), { target: { value: 'to_referrer' } })
    fireEvent.change(screen.getByLabelText(/修改日期/), { target: { value: '2026-06-05' } })
    fireEvent.click(screen.getByRole('button', { name: '保存修改' }))
    await waitFor(() =>
      expect(payApi.updatePayment).toHaveBeenCalledWith('E1', expect.objectContaining({ amount: 2500, direction: 'to_referrer', paid_at: '2026-06-05' })),
    )
  })

  it('+ 记支出：类型/金额/日期/方式/备注 → createPayment 写对 direction/case_id', async () => {
    renderCard([], [payment])
    ...
    await waitFor(() =>
      expect(payApi.createPayment).toHaveBeenCalledWith(
        expect.objectContaining({
          case_id: 'ca1', direction: 'misc_expense', amount: 350, currency: 'AUD',
          note: '体检费垫付', applicant_id: null, plan_item_id: null,
        }),
      ),
    )
