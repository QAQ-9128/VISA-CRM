...
describe('OccupationalDurationRows（概要带审理时长 · 职业评估两段紧凑版 · §5）', () => {
  const dur = (over: Partial<OccupationalDurations> = {}): OccupationalDurations => ({
    chn: { start: '2026-05-12', days: 29, ongoing: false },
    skill: { start: '2026-06-10', days: 20, ongoing: true },
    ...over,
  })

  it('两段都未发生 → 整格「—」', () => {
    render(<OccupationalDurationRows durations={{ chn: null, skill: null }} />)
    expect(screen.getByText('—')).toBeInTheDocument()
    expect(screen.queryByTestId('oa-dur-chn')).toBeNull()
  })

  it('两段都发生：CHN / Skill 各一行，显示天数（绿色等宽）', () => {
    render(<OccupationalDurationRows durations={dur()} />)
    expect(screen.getByTestId('oa-dur-chn')).toBeInTheDocument()
    expect(screen.getByTestId('oa-dur-skill')).toBeInTheDocument()
    expect(screen.getByText('29').className).toContain('text-emerald-700')
    expect(screen.getByText('20').className).toContain('tabular-nums')
  })

  it('部分发生：未发生那段显示「—」，已发生那段显示天数', () => {
    render(<OccupationalDurationRows durations={dur({ skill: null })} />)
    expect(screen.getByText('29')).toBeInTheDocument()
    const skillRow = screen.getByTestId('oa-dur-skill')
