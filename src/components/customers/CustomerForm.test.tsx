...
describe('CustomerForm（改进版 UI 行为）', () => {
  ...
  it('填中文名 + 保存 → onSubmit 收到 chinese_name + 派生 full_name + joinCaseId=null + next=detail', () => {
    ...
    expect(onSubmit.mock.calls[0][0]).toMatchObject({ full_name: '李雷', chinese_name: '李雷', english_name: null })
    expect(onSubmit.mock.calls[0][1]).toBeNull()
    expect(onSubmit.mock.calls[0][2]).toBe('detail')
  })

  it('只填英文名也可保存：english_name 原样写入（不改大小写），full_name=英文', () => {
    const { onSubmit } = renderForm()
    const save = screen.getByRole('button', { name: '保存' })
    expect(save).toBeDisabled()
  ...
  it('「保存并新建案件」（重录快捷路径）：next=new-case；名字为空时禁用', () => {
    const { onSubmit } = renderForm()
...
describe('CustomerForm「组（Group）」区（一案一组：加入已有案件）', () => {
  ...
  it('选「加入已有案件」→ 下拉列出真实案件（案件号+参与人+组码·N 人，归档案件不列）；选中 → 保存带 joinCaseId', async () => {
    ...
    fireEvent.change(screen.getByLabelText('中文名'), { target: { value: '新客户' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
  ...
  it('「+ 快速建档同组的人」：建好出 chip，提交时第 4 参带 TA 的 id；✕ 可移出名单', async () => {
    ...
    // 外层表单与建人块各有一组「中文名/英文名」输入：第二个是建人块的（同一 NameFields 组件）
    fireEvent.change(screen.getAllByLabelText('中文名')[1], { target: { value: '李四' } })
    fireEvent.click(screen.getByRole('button', { name: /创建并加入名单/ }))
    // chip 出现（显示名=中文优先）
    expect(await screen.findByText('李四')).toBeInTheDocument()

    fireEvent.change(screen.getAllByLabelText('中文名')[0], { target: { value: '张三' } })
  ...
  it('筛选可用（按参与人名）；取消勾选「加入已有案件」→ joinCaseId 清空（= 独立客户）', async () => {
    ...
    expect(screen.queryByText('选择案件')).not.toBeInTheDocument()
    fireEvent.change(screen.getByLabelText('中文名'), { target: { value: '独立客户' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
    expect(onSubmit.mock.calls[0][1]).toBeNull()
  })
})
