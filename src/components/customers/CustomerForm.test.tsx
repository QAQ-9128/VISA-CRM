...
describe('CustomerForm「组（Group）」区（一案一组：加入已有案件）', () => {
  ...
  it('默认「新建独立客户（自成一组）」选中，不显示「选择案件」选择器', () => {
    renderForm()
    expect(screen.getByLabelText(/新建独立客户（自成一组）/)).toBeChecked()
  ...
  it('筛选可用（按参与人名）；切回「新建独立客户」→ joinCaseId 清空', async () => {
    const { onSubmit } = renderForm()
    fireEvent.click(screen.getByLabelText(/加入已有案件/))
    ...
    fireEvent.click(screen.getByLabelText(/新建独立客户（自成一组）/))
