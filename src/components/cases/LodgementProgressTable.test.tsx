...
describe('审理时长列（常显 + 获批定格）+ 提名/签证状态列', () => {
  it('列头：「提名/签证审理时长」+「提名/签证状态」，不再有「距今」', () => {
    renderTable(row())
    expect(screen.getByText('提名审理时长')).toBeInTheDocument()
    expect(screen.getByText('签证审理时长')).toBeInTheDocument()
    expect(screen.getByText('提名状态')).toBeInTheDocument()
    expect(screen.getByText('签证状态')).toBeInTheDocument()
