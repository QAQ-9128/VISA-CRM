...
import { CASE_STAGES, CASE_STAGE_COLOR, CASE_STAGE_LABELS, CASE_STAGE_STYLES } from './domain'

describe('阶段配色：每个阶段一色（阶段进展链 / 流转记录 / 徽章共用，不允许撞色）', () => {
  it('CASE_STAGE_STYLES：全部正式阶段的类名互不重复', () => {
    const styles = CASE_STAGES.map((s) => CASE_STAGE_STYLES[s])
    expect(new Set(styles).size).toBe(CASE_STAGES.length)
  })

  it('CASE_STAGE_COLOR：全部正式阶段的实色互不重复', () => {
    const colors = CASE_STAGES.map((s) => CASE_STAGE_COLOR[s])
    expect(new Set(colors).size).toBe(CASE_STAGES.length)
  })

  it('等待付款（awaiting_payment）：紧排在已草拟之后，标签/徽章/实色齐备', () => {
    const i = (CASE_STAGES as readonly string[]).indexOf('drafted')
    expect(CASE_STAGES[i + 1]).toBe('awaiting_payment')
    expect(CASE_STAGE_LABELS.awaiting_payment).toBe('等待付款')
    expect(CASE_STAGE_STYLES.awaiting_payment).toBeTruthy()
    expect(CASE_STAGE_COLOR.awaiting_payment).toMatch(/^#[0-9a-f]{6}$/i)
  })
