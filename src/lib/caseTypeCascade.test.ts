...
describe('caseTypeCascade · visa_stream 派生（值=现有目录 stream，标签照 mock）', () => {
  it('482 stream 入库值与 VISA_CATALOG 一致（TRT 检测等既有逻辑不破坏）+ 新增「副申请」', () => {
    const vals = STREAM_OPTIONS['482']!.options.map((o) => o.value)
    expect(vals).toEqual(['Core Skills', 'Specialist Skills', 'Labour Agreement', SECONDARY_APPLICANT_STREAM])
    // 该选项显示名改为「Subsequent Entrant」；入库值仍为 'Secondary Applicant'（不变，旧数据不丢）
    const sec = STREAM_OPTIONS['482']!.options.find((o) => o.value === SECONDARY_APPLICANT_STREAM)
    expect(sec?.label).toBe('Subsequent Entrant')
    expect(cascadeStream('482', SECONDARY_APPLICANT_STREAM)).toBe(SECONDARY_APPLICANT_STREAM)
    // 反向回填：副申请是目录内值 → 编辑打开原样保留（不被「旧值打开即空」清掉）
    expect(cascadeFromCase(mkCase({ visa_subclass: '482', visa_stream: SECONDARY_APPLICANT_STREAM })).stream).toBe(
      SECONDARY_APPLICANT_STREAM,
    )
  })
...
describe('caseTypeCascade · 字段可见性配置（担保职位/雇主并入相关类型，不再常驻）', () => {
  ...
  it('showSponsorFields：482 选「副申请」隐藏担保职位/雇主；其它子类别/类型正常显示', () => {
    // 482 副申请 → 不显示担保字段（副申无自己的担保雇主/职位）
    expect(showSponsorFields('482', SECONDARY_APPLICANT_STREAM)).toBe(false)
    // 482 其它子类别 → 正常显示
    expect(showSponsorFields('482', 'Core Skills')).toBe(true)
    expect(showSponsorFields('482', 'Specialist Skills')).toBe(true)
    // 482 未选子类别（空）→ 仍显示（用户尚未选副申）
    expect(showSponsorFields('482', '')).toBe(true)
    // 其它类型不受影响：186/407 任意 stream 都显示；「副申请」字符串只对 482 生效
    expect(showSponsorFields('186', SECONDARY_APPLICANT_STREAM)).toBe(true)
    expect(showSponsorFields('407', '')).toBe(true)
  })
  ...
  it('VISA_TYPES 九项、key 唯一；407 紧随 186（同担保一组）；485 在册', () => {
    expect(VISA_TYPES).toHaveLength(9)
    expect(new Set(VISA_TYPES.map((t) => t.key)).size).toBe(9)
    ...
    expect(VISA_TYPES.find((t) => t.key === '485')?.label).toBe('485 毕业生工签')
  })

  it('485 毕业生工签：子类别含常规 stream + Subsequent Entrant；无担保字段；保存走现有列', () => {
    // 子类别下拉存在，且包含 Subsequent Entrant
    const opts485 = STREAM_OPTIONS['485']!.options
    expect(opts485.some((o) => o.value === SECONDARY_APPLICANT_STREAM && o.label === 'Subsequent Entrant')).toBe(true)
    expect(opts485.length).toBeGreaterThanOrEqual(2) // 至少 1 个常规 stream + Subsequent Entrant
    // 485 无担保职位/雇主
    expect(SPONSOR_TYPES.has('485')).toBe(false)
    expect(EMPLOYER_TYPES.has('485')).toBe(false)
    // showSponsorFields 对 485 恒 true（但本就不在 SPONSOR/EMPLOYER 集合，故不显示担保）
    expect(showSponsorFields('485', SECONDARY_APPLICANT_STREAM)).toBe(true)
    // 入库：visa_subclass=485；子类别存入 visa_stream（与其它类型同方式）
    expect(cascadeSubclass('签证申请', '485', 'Post-Study Work')).toBe('485')
    expect(cascadeStream('485', 'Post-Study Work')).toBe('Post-Study Work')
    expect(cascadeStream('485', SECONDARY_APPLICANT_STREAM)).toBe(SECONDARY_APPLICANT_STREAM)
    // 反向回填：visa_subclass=485 → 签证申请 / 485；目录内 stream 原样保留
    expect(cascadeFromCase(mkCase({ visa_subclass: '485', visa_stream: 'Graduate Work' }))).toMatchObject({
      category: '签证申请', visaType: '485', stream: 'Graduate Work',
    })
  })
})
