...
const mkCustomer = (o: Partial<Customer>): Customer => ({
  id: 'cu1', full_name: '张三', is_starred: false, client_source: null, tag: null, primary_applicant_id: null,
...
describe('selectFinancePayouts · 垫付杂项（misc_expense 第三支出流）', () => {
  it('misc_expense 单独计入 miscTotal，与付主代理/付介绍人分开；负数（冲红）夹 0 不计入、明细保留', () => {
    const cases = { c1: mkCase({}) }
    const customers = { cu1: mkCustomer({}) }
    const pays = [
      mkPayment({ id: 'm1', direction: 'misc_expense', amount: 350, paid_at: '2026-06-01', note: '翻译费垫付' }),
      mkPayment({ id: 'm2', direction: 'misc_expense', amount: -50, paid_at: '2026-06-02' }),
      mkPayment({ id: 'a', direction: 'to_company', amount: 3000, paid_at: '2026-06-03' }),
      mkPayment({ id: 'b', direction: 'to_referrer', amount: 800, paid_at: '2026-06-04' }),
    ]
    const p = selectFinancePayouts(pays, cases, customers, {})
    expect(p.miscTotal).toBe(350)
    expect(p.toCompanyTotal).toBe(3000)
    expect(p.toReferrerTotal).toBe(800)
    // 明细：日期倒序，负数行保留可追溯
    expect(p.items.map((i) => i.paymentId)).toEqual(['b', 'a', 'm2', 'm1'])
    expect(p.items.find((i) => i.paymentId === 'm1')?.direction).toBe('misc_expense')
  })

  it('垫付杂项不计入客户已收/已付（应收口径只认 from_client）', () => {
    const cases = [mkCase({ id: 'c1', customer_id: 'cu1' })]
    const customerById = { cu1: mkCustomer({ id: 'cu1' }) }
    const plans = [mkPlan({ id: 'p1', case_id: 'c1' })]
    const items = [mkItem({ id: 'i1', plan_id: 'p1', amount_due: 1000 })]
    const payments = [
      mkPayment({ id: 'a', case_id: 'c1', direction: 'from_client', amount: 300, plan_item_id: 'i1' }),
      mkPayment({ id: 'm', case_id: 'c1', direction: 'misc_expense', amount: 500 }),
    ]
    const rows = selectFinanceReceivables(cases, [], plans, payments, customerById, items)
    expect(rows[0]).toMatchObject({ receivable: 1000, paid: 300, unpaid: 700 })
  })
})
...
describe('月度账目合并流水（selectLedgerRows / filter / counts）', () => {
  ...
  const payouts = (items: PayoutItem[]): FinancePayouts => ({ items, toCompanyTotal: 0, toReferrerTotal: 0, miscTotal: 0 })
