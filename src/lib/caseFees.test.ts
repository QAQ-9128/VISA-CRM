...
const mkPay = (o: Partial<Payment>): Payment => ({
  ...
  is_shared: false, recorded_by: null, created_at: '', ...o,
})
...
describe('selectCaseFeeGroups — 应付款项(kind=payable)不进应收视图（数字与改前一致）', () => {
  it('同计划下混入应付款项 → 应收行/合计只算应收款项，应付被排除', () => {
    const caseRow = mkCase({ id: 'ca1', customer_id: 'P', sync_tracking: false })
    const out = selectCaseFeeGroups(
      caseRow,
      ['P'],
      [mkPlan({ id: 'pl1', case_id: 'ca1', applicant_id: 'P' })],
      [],
      custById([mkCust({ id: 'P', full_name: 'Alice' })]),
      [
        mkItem({ id: 'rec', plan_id: 'pl1', fee_category: '律师费', amount_due: 1000, kind: null }),
        mkItem({ id: 'pay', plan_id: 'pl1', fee_category: '提名代理费', amount_due: 5000, kind: 'payable' }),
      ],
    )
    // 只出应收行，应付款项不出现；应收合计 = 1000（不含 5000 应付）
    const labels = out.groups.flatMap((g) => g.lines.map((l) => l.label))
    expect(labels).toEqual(['律师费'])
    expect(out.totals).toEqual({ receivable: 1000, paid: 0, unpaid: 1000 })
  })
})

describe('selectCaseFeeGroups — 共享·全案（is_shared，不归任何 applicant）', () => {
  const customers = [mkCust({ id: 'P', full_name: 'Alice' })]
  const caseRow = mkCase({ id: 'ca1', customer_id: 'P', sync_tracking: false })
  const plan = mkPlan({ id: 'pl1', case_id: 'ca1', applicant_id: 'P' })
  const planNull = mkPlan({ id: 'plN', case_id: 'ca1', applicant_id: null }) // 共享款项挂 null 计划
  const items = [
    mkItem({ id: 'law', plan_id: 'pl1', fee_category: '律师费', amount_due: 1000, is_shared: false }),
    mkItem({ id: 'gov', plan_id: 'plN', fee_category: '政府申请费(全案)', amount_due: 500, is_shared: true }),
  ]
  const payments = [
    mkPay({ id: 'r1', direction: 'from_client', plan_item_id: 'law', applicant_id: 'P', amount: 1000, is_shared: false }),
    mkPay({ id: 'sh', direction: 'from_client', plan_item_id: 'gov', applicant_id: null, amount: 500, is_shared: true }),
  ]
  const out = selectCaseFeeGroups(caseRow, ['P'], [plan, planNull], payments, custById(customers), items)

  it('共享款项独立成 sharedGroup（shared=true / 组名「共享 · 全案」），不并进客户组', () => {
    expect(out.sharedGroup.shared).toBe(true)
    expect(out.sharedGroup.participantName).toBe('共享 · 全案')
    expect(out.sharedGroup.lines.map((l) => l.label)).toEqual(['政府申请费(全案)'])
    expect(out.sharedGroup).toMatchObject({ receivable: 500, paid: 500, unpaid: 0, applicantId: null })
    // 客户组（Alice）只含自己的律师费，绝不吸收共享款项（即便共享款项挂在 null 计划、owner 覆盖 null）
    expect(out.groups.map((g) => g.participantName)).toEqual(['Alice'])
    expect(out.groups[0].lines.map((l) => l.label)).toEqual(['律师费'])
    expect(out.groups[0]).toMatchObject({ receivable: 1000, paid: 1000, unpaid: 0 })
    expect(out.groups.some((g) => g.shared)).toBe(false)
  })

  it('🔒 本案合计 = 客户组 + 共享组（应收 1500 / 已收 1500 / 未收 0）；与 getCaseTotals(全部 items+payments) 同口径', () => {
    expect(out.totals).toEqual({ receivable: 1500, paid: 1500, unpaid: 0 })
    const t = getCaseTotals(items, payments) // 算法未改：全部款项（含共享）求和
    expect(out.totals).toEqual({ receivable: t.totalDue, paid: t.totalPaid, unpaid: Math.max(0, t.totalUnpaid) })
  })

  it('无共享款项：sharedGroup 恒在（空、供「添加款项」入口）但小计为 0，不影响合计', () => {
    const noShared = selectCaseFeeGroups(caseRow, ['P'], [plan], [payments[0]], custById(customers), [items[0]])
    expect(noShared.sharedGroup.shared).toBe(true)
    expect(noShared.sharedGroup.lines).toEqual([])
    expect(noShared.sharedGroup).toMatchObject({ receivable: 0, paid: 0, unpaid: 0 })
    expect(noShared.totals).toEqual({ receivable: 1000, paid: 1000, unpaid: 0 })
    expect(noShared.multi).toBe(false) // 共享组不算进 multi（multi 只看客户组）
  })
})
