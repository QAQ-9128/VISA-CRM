...
/** 取消归档（回收站恢复）。 */
export async function unarchiveEmployer(id: string): Promise<void> {
  const { error } = await supabase.from('employers').update({ is_archived: false }).eq('id', id)
  if (error) throw error
}

/** 彻底删除（硬删，不可恢复）：真 DELETE。已挂靠客户的 sponsor_employer_id 会被置空。RLS 仅 admin。 */
