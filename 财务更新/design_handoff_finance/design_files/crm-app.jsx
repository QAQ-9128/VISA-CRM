        ...
        <td>
          <div style={{display:'flex',alignItems:'center',gap:11}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:7,flexWrap:'wrap'}}>
              <span style={{fontSize:14,fontWeight:600,color:'var(--brand)',cursor:'pointer'}} onClick={()=>go('#/cases/detail')}>{r.name}</span>
              {r.merged && <span className="mtag">合并</span>}
              <span style={{fontSize:12,color:'var(--faint)'}}>· {r.visa}</span>
              {hasStages && <span className="phase-tag" onClick={()=>setOpen(o=>!o)}>分 {r.stages.length} 期<Icon name="chevron" size={11} style={{transform:open?'rotate(-90deg)':'rotate(90deg)'}}/></span>}
            </div>
          </div>
        </td>
        <td className="num"><PayCell received={r.received} billed={r.billed}/></td>
        <td><StatusCell r={r}/></td>
        <td style={{textAlign:'right'}}>{hasStages ? <span className="link" onClick={()=>setOpen(o=>!o)}>{open?'收起':'展开'}</span> : <RecMenu/>}</td>
      </tr>
      {open && hasStages && <tr><td colSpan={4} style={{padding:0,borderTop:0,background:'var(--surface-2)'}}><StagePanel r={r}/></td></tr>}
    </React.Fragment>
  );
}
function FinancePage(){
  ...
  return (
    <AppShell active="#/finance" title="财务" sub="应收快照 + 月度收付明细" width="route-md">
      {/* 近期案件应收 */}
      <Card pad={false}>
        <div style={{padding:'22px 22px 0',display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:14}}>
          <div><h3 style={{margin:0,fontSize:16,fontWeight:700,color:'var(--ink)'}}>近期案件应收</h3>
            <div className="hsub" style={{marginTop:3}}>当下应收状态,与下方月份筛选无关</div></div>
        </div>
        <div style={{overflowX:'auto',marginTop:8}}>
          <table className="tbl">
            <thead><tr><th>客户 · 案件</th><th className="num">已付 / 应收</th><th>未付 / 状态</th><th style={{textAlign:'right'}}>操作</th></tr></thead>
            <tbody>
              {f.receivables.map((r,i)=><RecRow key={i} r={r}/>)}
              <tr style={{borderTop:'2px solid var(--line-2)'}}>
                <td style={{fontWeight:700,color:'var(--ink)'}}>合计</td>
                <td className="num"><span style={{fontVariantNumeric:'tabular-nums',fontSize:13.5}}><b style={{color:'var(--emerald)'}}>{money(tRecv)}</b> <span style={{color:'var(--faint)'}}>/ {money(tBilled)}</span></span></td>
